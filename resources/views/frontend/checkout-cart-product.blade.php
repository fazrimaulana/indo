...
@section('content')
	...
	<form method="post" action="{{ url('/cart/carts/payment/order') }}" class="form-horizontal" autocomplete="off">
  {{ csrf_field() }}
                     ...
                     <div class="panel-body list-cart">
                      ...
                      @foreach($carts as $cart)
                        @php
                          ...
                          $weightProduct += $product->weight * $cart->quantity;
                        @endphp
                        <div class="list row">
                            ...
                            <div class=" col-md-4 col-sm-4 col-xs-4">
                                <div class="harga pull-right">
                                    ...
                                    @if($product->discount!=0 && $product->start_time_discount<=date('Y-m-d H:i:s') && $product->end_time_discount>=date('Y-m-d H:i:s'))
                                      ...
                                      Rp. {{ App\Helpers\Money::setRupiah( ($product->price - ( ($product->price*$product->discount)/100 ) ) * $cart->quantity ) }}

                                      @php
                                        $totalHargaBarang+= ( $product->price - ( ( $product->price*$product->discount )/100 ) ) * $cart->quantity;
                                      @endphp

                                    @else
                                      Rp. {{ App\Helpers\Money::setRupiah($product->price * $cart->quantity) }}

                                      @php
                                        $totalHargaBarang += $product->price * $cart->quantity;
                                      @endphp
               ...
               <div class="left col-md-6 col-sm-6 pull-left">
                   <div class="panel panel-default shipping">
                      ...
                      <div class="panel-body">
                          ...
                          @foreach($carts as $cart)
                            <input type="hidden" name="idProduct[]" value="{{ $cart->id }}">
